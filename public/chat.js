...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (saveSettingsBtn) {
    saveSettingsBtn.addEventListener("click", () => {
      // Save status
      if (typeof statusInput !== "undefined" && statusInput) {
        const status = statusInput.value.trim();
        localStorage.setItem("userStatus", status);
        if (document.getElementById("status")) {
          document.getElementById("status").textContent = `● ${status || 'Online'}`;
      ...
      // Save profile photo
      if (typeof photoInput !== "undefined" && photoInput && photoInput.files[0]) {
        const reader = new FileReader();
        reader.onload = () => {
          localStorage.setItem("photo", reader.result);
          if (document.getElementById("userPic"))
...
if (saveSettingsBtn) {
  saveSettingsBtn.addEventListener("click", () => {
    ...
    // Save status
    if (statusInputEl) {
      const status = statusInputEl.value.trim();
      localStorage.setItem("userStatus", status);
      const statusDisplay = document.getElementById("status");
      const profileStatusEl = document.getElementById("profileStatus");
      if (statusDisplay) statusDisplay.textContent = `● ${status || "Online"}`;
      if (profileStatusEl) profileStatusEl.textContent = status || "Online";
    }
    ...
    if (photoInputEl && photoInputEl.files && photoInputEl.files[0]) {
      ...
      reader.onload = () => {
        ...
        const profilePicLargeEl = document.getElementById("profilePicLarge");
        if (userPicEl) userPicEl.src = reader.result;
        if (profilePicLargeEl) profilePicLargeEl.src = reader.result;
        alert("Settings saved!");
        if (settingsPanelEl) settingsPanelEl.style.display = "none";
      };
      reader.readAsDataURL(photoInputEl.files[0]);
    } else {
      alert("Settings saved!");
      if (settingsPanelEl) settingsPanelEl.style.display = "none";
    }
